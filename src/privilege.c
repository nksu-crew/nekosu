...
void grant_privileges(unsigned int flags, kernel_cap_t caps_to_raise, const char *target_domain)
{
    struct cred *new_cred;
    bool needs_commit = false;

    /* seccomp-only fast path: no cred changes needed */
    if ((flags & PRIV_SECCOMP) && !(flags & (PRIV_ROOT | PRIV_CAPS | PRIV_SELINUX))) {
        disable_seccomp();
        return;
    }
